...
function ChatPage() {
  ...
  const send = (text: string) => {
    setCustomMsgs((m) => [...m, { role: "user", text }, { role: "ai", agent: "general", text: t("gotItAnalysing") }]);
  };
  ...
  return (
    <div className="min-h-screen flex" style={{ background: "#fff" }}>
      ...
      <div className="flex-1 md:pl-[240px] flex pb-16 md:pb-0">
        <aside className="hidden md:flex flex-col border-r" style={{ width: 280, borderColor: "#e5e7eb" }}>
          <div className="px-4 h-14 flex items-center border-b" style={{ borderColor: "#e5e7eb", fontSize: 13, fontWeight: 500 }}>
            {t("chats")}
          </div>
          <div className="p-2 space-y-1 overflow-y-auto">
            {history.map((h, i) => (
              <button key={i} className="w-full text-left px-3 py-2 rounded-md"
                style={{ background: i === 0 ? "#f0f5ea" : "transparent" }}
                onMouseEnter={(e) => { if (i !== 0) e.currentTarget.style.background = "#f7f8f6"; }}
                onMouseLeave={(e) => { if (i !== 0) e.currentTarget.style.background = "transparent"; }}>
                <div className="truncate" style={{ fontSize: 13, color: "#1a1a1a" }}>{h.title}</div>
        ...
        <main className="flex-1 flex flex-col">
          <div className="h-14 px-6 flex items-center justify-between border-b" style={{ borderColor: "#e5e7eb" }}>
            <AgentPill type="general" />
            <LanguageSelector value={lang} onChange={setLang} />
          </div>
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4 max-w-3xl mx-auto w-full">
            {allMsgs.map((m, i) => <ChatBubble key={i} {...m} />)}
          </div>
