  ...
  return (
    <PageWrapper title={t("dashboard")}>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <MetricCard label={t("activeCrop")} value={t("tomato")} sub={t("sownDateVal")} icon={<Sprout size={14} strokeWidth={1.75} />} />
        <MetricCard
          label={t("lastDiagnosis")}
          value={<span className="flex items-center gap-2">{t("leafBlight")} <SeverityBadge level="med" /></span>}
          sub={t("lastDiagVal")} icon={<Activity size={14} strokeWidth={1.75} />}
        />
        <MetricCard label={t("nextIrrigation")} value={t("in2Days")} sub={t("morning16May")} icon={<Droplets size={14} strokeWidth={1.75} />} />
        <MetricCard
          label={t("mandiPrice")}
          value={<span className="flex items-center gap-2">₹2,340 <span style={{ color: "#639922", fontSize: 12 }} className="flex items-center"><ArrowUp size={12} />4.2%</span></span>}
          sub={`${t("perQuintal")} · Kolar`} icon={<TrendingUp size={14} strokeWidth={1.75} />}
        />
      </div>
            ...
            {activity.map((a, i) => (
              <li key={i} className="py-2.5 flex items-center gap-3">
                <div className="flex-1 min-w-0 flex items-center gap-3">
                  ...
                  <span className="truncate" style={{ fontSize: 13, color: "#1a1a1a" }}>{t(a.qKey)}</span>
                </div>
        ...
        <Card className="lg:col-span-2">
          ...
          <div className="grid grid-cols-5 gap-2 mt-3">
            {forecast.map((f) => {
              const Icon = f.icon;
              return (
                <div key={f.dayKey} className="rounded-lg p-2 text-center"
                     style={{ border: "1px solid #e5e7eb", background: "#fff" }}>
                  <div style={{ fontSize: 11, color: "#6b7280" }}>{t(f.dayKey)}</div>
                  <Icon size={20} strokeWidth={1.5} style={{ color: "#6b7280", margin: "8px auto" }} />
                  <div style={{ fontSize: 12, color: "#1a1a1a" }}>{f.hi}°<span style={{ color: "#6b7280" }}>/{f.lo}°</span></div>
                </div>
              );
            })}
          </div>
