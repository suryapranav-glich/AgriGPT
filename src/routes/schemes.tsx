...
const filterKeys = [
  "all", "subsidies", "seeds", "insurance", "credit", "irrigationTab",
] as const;

const schemesList = [
  { nameKey: "pmKisanName",    stateKey: "central",   categoryKey: "subsidies",    summaryKey: "pmKisanSum",    url: "https://pmkisan.gov.in" },
  { nameKey: "pmfbyName",      stateKey: "central",   categoryKey: "insurance",    summaryKey: "pmfbySum",      url: "https://pmfby.gov.in" },
  { nameKey: "kccName",        stateKey: "central",   categoryKey: "credit",       summaryKey: "kccSum",        url: "https://www.nabard.org/content1.aspx?id=572" },
  { nameKey: "pmksyName",      stateKey: "central",   categoryKey: "irrigationTab",summaryKey: "pmksySum",      url: "https://pmksy.gov.in" },
  { nameKey: "raithaSiriName", stateKey: "karnataka", categoryKey: "subsidies",    summaryKey: "raithaSiriSum", url: "https://raitamitra.karnataka.gov.in" },
  { nameKey: "seedMissionName",stateKey: "central",   categoryKey: "seeds",        summaryKey: "seedMissionSum",url: "https://seednet.gov.in" },
];
...
function Schemes() {
  ...
  const list = active === "all"
    ? schemesList
    : schemesList.filter((s) => s.categoryKey === active);
    ...
    <PageWrapper title={t("govtSchemes")}>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        >
          ...
          {q && (
            <button
              onClick={handleClear}
              style={{ fontSize: 18, color: "#9ca3af", lineHeight: 1 }}
            >×</button>
          )}
        </div>
      ...
      {loading && (
        <div
          ...
          style={{ background: "#fff", border: "1px solid #e5e7eb" }}
        >
          <div className="h-3 rounded" style={{ background: "#f3f4f6", width: "60%", marginBottom: 10 }} />
          <div className="h-3 rounded" style={{ background: "#f3f4f6", width: "90%", marginBottom: 8 }} />
      ...
      {!loading && answer && (
        <div
          ...
          style={{ background: "#fff", border: "1px solid #e5e7eb", borderTop: "3px solid #3b6d11", overflow: "hidden" }}
        >
          {/* Header */}
              ...
              <div>
                <span
                  style={{
                    ...
                    background: "#f0f5ea", color: "#3b6d11",
                    borderRadius: 4, padding: "2px 8px", marginBottom: 6,
                  }}
                >{answer.scheme_name}</span>
                ...
                {answer.amount_or_benefit && (
                  <span
                    style={{
                      ...
                      borderRadius: 4, padding: "2px 8px", marginBottom: 6, marginLeft: 6,
                    }}
                  >{answer.amount_or_benefit}</span>
                )}
              </div>
            </div>
          ...
          {expanded && (
            <div className="px-4 py-3" style={{ borderTop: "1px solid #f3f4f6" }}>
              ...
              {/* Eligibility */}
              <div className="mb-3">
                <p style={{ fontSize: 11, fontWeight: 700, color: "#6b7280", textTransform: "uppercase", marginBottom: 4 }}>
                  Who is eligible
                </p>
                <p style={{ fontSize: 13, color: "#374151", lineHeight: 1.55 }}>{answer.eligibility}</p>
              </div>
              ...
              <div className="mb-3">
                ...
                <p style={{ fontSize: 13, color: "#374151", lineHeight: 1.55 }}>{answer.how_to_apply}</p>
              </div>
                ...
                <div className="mb-3">
                  ...
                  <ul style={{ margin: 0, paddingLeft: 16 }}>
                    {answer.documents_needed.map((doc, i) => (
                      <li key={i} style={{ fontSize: 13, color: "#374151", marginBottom: 2 }}>{doc}</li>
                    ))}
                  </ul>
                </div>
              )}
              ...
              {/* State-specific */}
              {answer.state_specific && answer.state_specific !== "Applies across all Indian states" && (
                <div
                  className="mb-3 rounded-lg p-3"
                  style={{ background: "#f0f5ea", border: "1px solid #d1e8b0" }}
                >
                  <p style={{ fontSize: 11, fontWeight: 700, color: "#3b6d11", textTransform: "uppercase", marginBottom: 3 }}>
                    State-specific note
                  </p>
              ...
              {answer.tip && (
                ...
                >
                  <p style={{ fontSize: 11, fontWeight: 700, color: "#b45309", textTransform: "uppercase", marginBottom: 3 }}>
                    Quick tip
                  </p>
              ...
              {answer.helpline && (
                <div className="mb-3 flex items-center gap-2">
                  <p style={{ fontSize: 11, fontWeight: 700, color: "#6b7280", textTransform: "uppercase" }}>Helpline:</p>
                  <p style={{ fontSize: 13, color: "#1a1a1a", fontWeight: 500 }}>{answer.helpline}</p>
                </div>
              )}
            </div>
          )}
          ...
          >
            ...
            {answer._meta?.pdf_chunks_used > 0 && (
              <span style={{ fontSize: 10, color: "#9ca3af" }}>
                · {answer._meta.pdf_chunks_used} PDF chunk{answer._meta.pdf_chunks_used > 1 ? "s" : ""} used
              </span>
            )}
          </div>
        </div>
