...
function SignUp() {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (password !== confirm) { setErr(t("passwordsDoNotMatch")); return; }
    signup({ name, email, language: lang, location: "India" });
    navigate({ to: "/" });
  };

  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-8" style={{ background: "var(--c-surface)" }}>
      <div className="w-full max-w-[420px] rounded-2xl p-8 relative"
        ...
        <form className="mt-6 space-y-3" onSubmit={submit}>
          <div>
            ...
            <div className="mt-1.5"><Input value={name} onChange={(e) => setName(e.target.value)} required /></div>
          </div>
          <div>
            <Label>{t("email")}</Label>
            <div className="mt-1.5"><Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required /></div>
          </div>
          <div>
            <Label>{t("password")}</Label>
            <div className="mt-1.5"><Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required /></div>
          </div>
          <div>
            <Label>{t("confirmPassword")}</Label>
            <div className="mt-1.5"><Input type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)} required /></div>
          </div>
          {err && <div style={{ fontSize: 12, color: "#e24b4a" }}>{err}</div>}
          <Button type="submit" className="w-full mt-2">{t("createAccount")}</Button>
        </form>
        ...
        <button
          onClick={() => { loginWithGoogle(); navigate({ to: "/" }); }}
        ...
        <p className="text-center mt-6" style={{ fontSize: 13, color: "var(--c-muted)" }}>
          {t("haveAccount")} <Link to="/signin" style={{ color: "#3b6d11" }}>{t("signIn")}</Link>
        </p>
      </div>
    </div>
  );
